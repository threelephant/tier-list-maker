...
Analytics is skipped automatically when your browser sends **Do-Not-Track** or **Global Privacy Control**. To opt out explicitly, untick **Send anonymous usage analytics** in the footer on the home page (or run `localStorage.setItem("analytics:off","1")` in the console). Events are auto-deleted after 90 days. Run [supabase/schema.sql](supabase/schema.sql) to create the `events` table (and, with the `pg_cron` extension enabled, the daily retention job) when self-hosting.
